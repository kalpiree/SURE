...
def construct_prediction_set(
    ...
    selected = [(item, score) for item, score in zip(candidate_items, normalized_scores) if score >= lambda_t]
...
def construct_prediction_set_at_step(
    ...
    for idx, row in df_at_step.iterrows():
        ...
        pred_set = construct_prediction_set(row, lambda_t, max_pred_set_size)
